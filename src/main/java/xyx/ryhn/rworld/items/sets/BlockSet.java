...
public class BlockSet {
	...
	public BlockSet(Block parent, String name, boolean isWood) {
		...
		BLOCKS.add(RWorldItems.registerBlock(SLAB, name + "_slab", ItemGroups.BUILDING_BLOCKS));
		bfb.slab(SLAB);

		STAIRS = new StairsBlock(PARENT.getDefaultState(), Settings.copy(PARENT));
		BLOCKS.add(RWorldItems.registerBlock(STAIRS, name + "_stairs", ItemGroups.BUILDING_BLOCKS));
		bfb.stairs(STAIRS);
		...
		BLOCKS.add(RWorldItems.registerBlock(PRESSURE_PLATE, name + "_pressure_plate", ItemGroups.REDSTONE));
		bfb.pressurePlate(PRESSURE_PLATE);
			...
			.noCollision()
			.pistonBehavior(PistonBehavior.DESTROY));
		BLOCKS.add(RWorldItems.registerBlock(BUTTON, name + "_button", ItemGroups.REDSTONE));
		bfb.button(BUTTON);
		...
		if (isWood) {
			...
			DOOR = new DoorBlock(SET_TYPE, Settings.copy(PARENT)
					.nonOpaque()
					.pistonBehavior(PistonBehavior.DESTROY));
			BLOCKS.add(RWorldItems.registerBlock(DOOR, name + "_door", ItemGroups.BUILDING_BLOCKS));
			bfb.door(DOOR);

			TRAPDOOR = new TrapdoorBlock(SET_TYPE, Settings.copy(PARENT)
					.nonOpaque()
					.pistonBehavior(PistonBehavior.DESTROY));
			BLOCKS.add(RWorldItems.registerBlock(TRAPDOOR, name + "_trapdoor", ItemGroups.BUILDING_BLOCKS));
			bfb.trapdoor(TRAPDOOR);

			FENCE = new FenceBlock(Settings.copy(Blocks.OAK_FENCE));
			BLOCKS.add(RWorldItems.registerBlock(FENCE, name + "_fence", ItemGroups.BUILDING_BLOCKS));
			bfb.fence(FENCE);

			FENCE_GATE = new FenceGateBlock(WOOD_TYPE, Settings.copy(Blocks.OAK_FENCE_GATE));
			BLOCKS.add(RWorldItems.registerBlock(FENCE_GATE, name + "_fence_gate", ItemGroups.BUILDING_BLOCKS));
			bfb.fenceGate(FENCE_GATE);
		} else {
			WALL = new WallBlock(Settings.copy(PARENT));
			BLOCKS.add(RWorldItems.registerBlock(WALL, name + "_wall", ItemGroups.BUILDING_BLOCKS));
			bfb.wall(WALL);
		}
